...
<script>
import edit from '@/views/sampling/majorTask/edit.vue'
...
import {getMainTaskQueryPageList, getMainTaskToOper, getChangeContCaseError} from '@/api/sampling/majorTask.js'
...
export default {
  ...
  methods: {
    ...
    handleAdd(params) {
      ...
      // XMLX_8  农业送样检测
      data.forEach(xdd => {
        if (xdd === 'XMLX_4' || xdd === 'XMLX_8' || xdd === 'XMLX_6') {
          ids += xdd + ','
        }
      })
