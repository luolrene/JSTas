<template>
  <homeRecord
    ...
    <F187Son :params="fromValiData"></F187Son>
  </homeRecord>
</template>

<script>
import projectList from '@/views/common/project_list.vue'
import F187Son from '../F_187_son.vue'
...
export default {
  ...
  data () {
    return {
      ...
      pointValue: '', // 选中的点位
      fromItemList: [
        {label: '浑浊度单位', prop: 'zd1', value: '', type: 'input', isRqd: true},
        ...
        {label: '采样日期', prop: 'zd2', value: '', type: 'date', isRqd: true},
  ...
  watch: {
    ...
    'fromValiData.zd1': {
      handler (newName, oldName) {
        ...
        let obj = this.tableHeader[5].data.find(xdd => xdd.prop === 'zd11')
        obj.label = '浑浊度' + '(' + newName + ')'
      },
      immediate: true,
      deep: true
    }
