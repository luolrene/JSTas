...
<template>
  <div class="operate-container">
    ...
    <el-tag
      ...
      closable
      v-show="selectPerson.name"
      @close="closePerson">{{selectPerson.name}}</el-tag>
    <tableItem
    :obj="this"
    :tableData="tableData"
...
export default {
  ...
  methods: {
    ...
    doConfirm () {
      if (!this.selectPerson.hasOwnProperty('name')) {
        this.$share.message('请点击表格选择人员', 'warning')
        return
      }
      this.$parent.getUserData(this.selectPerson, this.layerid)
    }
  },
  mounted () {
    if (this.checkType) {
      this.fromValiData.checkType = this.checkType
    }
    this.getListData()
    this.getdictData()
