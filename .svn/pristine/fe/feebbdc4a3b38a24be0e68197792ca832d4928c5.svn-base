<template>
  <homeRecord
    ...
    <div style="width: 100%;text-align: center;margin-bottom: 5px;font-size: 15px;color: #0195DB;">实际水样测试</div>
    <tableRecord
    :obj="this"
    :tableData="tableData"
    ...
    <F113Son :params="fromValiData"></F113Son>
  </homeRecord>
</template>
...
import F113Son from '../F_113_son.vue'
...
export default {
  ...
  data () {
    return {
      ...
      pointValue: '', // 选中的点位
      fromItemList: [
        {label: '在线仪器型号及名称', prop: 'zd1', value: '', type: 'input', isRqd: true, click: 'handleMachine'},
        ...
        {label: '在线方法原理', prop: 'zd7', value: '', type: 'input', isRqd: true},
        {label: '自动仪器测量范围', prop: 'zd8', value: '', type: 'input', isRqd: true},
        {label: '在线仪器测值单位(副表)', prop: 'zd9', value: '', type: 'input', isRqd: true},
        {label: '实验室测试结果单位(副表)', prop: 'zd10', value: '', type: 'input', isRqd: true},
        {label: '绝对误差单位(副表)', prop: 'zd11', value: '', type: 'input', isRqd: true},
        {label: '检控记录', prop: 'zd12', value: '', type: 'input', isRqd: true},
        {label: '采样人员', prop: 'zd50', value: '', type: 'select', isRqd: true, data: [], multiple: true, isNoClear: true, filterable: true},
        {label: '检测日期', prop: 'zd13', value: '', type: 'date', isRqd: true},
        {label: '检测指标', prop: 'zd16', value: '', type: 'select', isRqd: true, data: [], isNoClear: true, filterable: true},
        {label: '质控样配制信息', prop: 'zd14', value: '', type: 'textarea', isRqd: true},
  ...
  methods: {
    ...
    getMachineData(data) {
      ...
      data.forEach(xdd => {
        // 仪器名称型号
        zd1 += xdd.name + xdd.machineXh + ';'
        // 仪器编号
        zd2 += xdd.machineNo + ';'
      })
      this.$set(this.fromValiData, 'zd1', zd1.substring(0, zd1.length - 1))
  ...
  watch: {
    ...
    'fromValiData.zd9': {
      handler (newName, oldName) {
        ...
        let obj = this.tableHeader.find(xdd => xdd.prop === 'zd4')
