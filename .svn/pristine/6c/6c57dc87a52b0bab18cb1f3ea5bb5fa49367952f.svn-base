...
export default {
  ...
  data () {
    return {
      ...
      fromItemList: [
        ...
        {label: '被测量单位名称', prop: 'zd2', value: '', type: 'input', isRqd: true},
        {label: '接待人', prop: 'zd4', value: '', type: 'input', isRqd: true},
        {label: '联系电话', prop: 'zd25', value: '', type: 'input', isRqd: true},
        {label: '地址', prop: 'zd3', value: '', type: 'input', isRqd: true},
        ...
        {label: '校准仪器-名称', prop: 'zd9', value: '', type: 'input', isRqd: true, click: 'handleMachine'},
        {label: '校准仪器-型号', prop: 'zd10', value: '', type: 'input', isRqd: true},
        {label: '校准仪器-编号', prop: 'zd11', value: '', type: 'input', isRqd: true},
        {label: '校准仪器-有效期至', prop: 'zd12', value: '', type: 'input', isRqd: true},
        {label: '仪器测量前后校准情况-测量前（dB）', prop: 'zd13', value: '', type: 'input', isRqd: true},
        {label: '仪器测量前后校准情况-测量后（dB）', prop: 'zd14', value: '', type: 'input', isRqd: true},
        {label: '气象条件雨雪',
          prop: 'zd15',
          value: '',
          type: 'select',
        ...
        {label: '检测日期', prop: 'zd18', value: '', type: 'date', isRqd: true},
        {label: '测点示意图', prop: 'zd19', value: '', type: 'draw', isRqd: true},
        {label: '备注', prop: 'zd20', value: '', type: 'textarea', isRqd: true, row: 2}
      ],
  ...
  methods: {
    ...
    getMachineData(data) {
      ...
      } else if (this.presentMachine === 'zd9') {
        let zd9 = ''
        let zd10 = ''
        let zd11 = ''
        let zd12 = ''
      ...
      if (this.pointValue === '') {
        ...
        getSamplingTaskQuerySampNoPage(ids).then(res => {
          res.result.pageList.forEach(xdd => {
            this.tableData.push({id: null, zd1: xdd.pointName + '(' + xdd.pointNo + ')', zd50: xdd.sampNo, edit: false})
          })
        })
      }
    },
    ...
    commonContent() {
      if (this.fromValiData.id === null) {
        ...
        this.fromValiData.zd25 = this.fromValiData.contactsMobile
        this.fromValiData.zd3 = this.fromValiData.address
        this.$set(this.fromValiData, 'zd20', '道路等级：1、城市快速路；2、城市主干道；3、城市次干道；4、城市含路面轨道交通的道路；5、穿过城市的高速公路；6、其他道路。')
      }
