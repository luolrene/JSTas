...
import { getCustQueryPageData } from '@/api/client/info.js' // 获取客户信息
import {
...
export default {
  ...
  methods: {
    ...
    // 获取客户信息
    getCustData() {
      this.fromData.other = '2'
      getCustQueryPageData(this.fromData).then(res => {
        let custData = res.result.pageList.map(item => {
          return {
            id: item.id,
            name: item.name
          }
