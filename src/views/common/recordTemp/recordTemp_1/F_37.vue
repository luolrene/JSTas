  ...
  data () {
    return {
      ...
      fromItemList: [
        ...
        {label: '检测日期', prop: 'zd1', value: '', type: 'date', isRqd: true},
        {label: '浑浊度单位', prop: 'zd2', value: '', type: 'input', isRqd: true},
        {label: '采样示意图', prop: 'zd3', value: '', type: 'draw', isRqd: true},
        {label: '备注', prop: 'zd4', value: '', type: 'textarea', isRqd: true}
      ],
      ...
      if (this.pointValue === '') {
        ...
        getSamplingTaskQuerySampNoPage(ids).then(res => {
          res.result.pageList.forEach(xdd => {
            this.tableData.push({id: null, zd50: xdd.sampNo, zd4: xdd.pointName + '-' + xdd.pointNo, edit: false})
          })
        })
      }
    },
    ...
    'fromValiData.zd2': {
      handler (newName, oldName) {
        ...
        this.tableHeader.forEach(xdd => {
          if (xdd.label === '现场测定记录') {
            xdd.data.forEach(arc => {
              if (arc.prop === 'zd13') {
                arc.label = '浑浊度' + '(' + newName + ')'
              }
            })
          }
        })
