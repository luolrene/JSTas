...
import {getIndusQueryPageData, getIndusDelIndus, getIndusQueryIndusTree} from '../../../api/jcxxgl/msgDefend.js'
export default {
  components: {
  ...
  data () {
    return {
      ...
      fromData: [
        { type: 'input', prop: 'name', label: '行业名称' }
      ],
      paramsData: {
        father: 0
      },
  ...
  methods: {
    getListData (id) {
      ...
      getIndusQueryPageData(this.fromValiData).then(res => {
    ...
    handleDelete (row) {
      let that = this
      this.$share.confirm({
        message: '此操作将永久删除该数据及其' + '<span style="color:red;font-weight:700">子数据</span>' + ', 是否继续?',
        confirm: function () {
          getIndusDelIndus({id: row.id}).then(res => {
